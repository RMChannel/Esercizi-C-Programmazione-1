...
int main(void) {
    int numeri[N]={0}, somma=0, max, min, crescente=1, decrescente=1, pari=0, dispari=0, i3;
    for (i3=0;i3<4;i3++) {
        int temp;
    ...
    for (int i=0;i<i3;i++) {
        ...
        if ((numeri[i]>=numeri[(i+1)]) && (crescente) && ((i+1)<i3)) crescente=0;
        if ((numeri[i]<=numeri[(i+1)]) && (decrescente) && ((i+1)<i3)) decrescente=0;
